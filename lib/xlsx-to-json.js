...
module.exports = {
  ...
  toJson: function(src, dest) {
    ...
    // codepage for csv!!!
    // convert utf-8 to utf-8 with bom
    let workbook = xlsx.parse(src, { codepage: 65001 });
    parseWorkbook(src, workbook, dest, headIndex, false);
  },
};
 ...
 */
function parseWorkbook(src, workbook, dest, headIndex, uglifyJson) {
  workbook.forEach(sheet => {
    ...
    if (sheet.name.indexOf('@') === -1 && sheet.name[0] !== '#') {
      ...
      let dest_file = path.resolve(
        dest,
        filename.substring(0, filename.length - 4) + '.json',
      );
      let formatedJson = JSON.stringify(parsedSheet, null, uglifyJson ? 0 : 2);
